...
func TestKeys(t *testing.T) {
	...
	cases := TestCases{
		...
		},
		{
			...
			"Wrap With KeyVersionID",
			func(t *testing.T, api *API, ctx context.Context) error {
				MockAuthURL(keyURL, http.StatusOK, keyActionWithKeyVersion)
				MockAuthURL(keyURL, http.StatusOK, keyActionWithKeyVersion)

				wrapWithKeyVersion, err := api.WrapWithKeyVersion(ctx, testKey, []byte(keysActionPT.PlainText), nil)
				assert.NoError(t, err)
				assert.NotEqual(t, wrapWithKeyVersion.PlainText, wrapWithKeyVersion.CipherText)

				plainText, err := api.Unwrap(ctx, testKey, []byte(wrapWithKeyVersion.CipherText), nil)
				assert.NoError(t, err)
				assert.Equal(t, wrapWithKeyVersion.PlainText, string(plainText))

				MockAuthURL(keyURL, http.StatusOK, testKeyVersions)

				keyVersion, err := api.ListKeyVersions(ctx, testKey, nil)
				assert.NoError(t, err)
				assert.Equal(t, testKey, keyVersion.KeyVersion[0].ID)
				assert.Equal(t, wrapWithKeyVersion.KeyVersionDetails.ID, keyVersion.KeyVersion[0].ID)
