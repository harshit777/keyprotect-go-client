...
func TestListKeySearch(t *testing.T) {
	...
	var searchObj KeySearchParam
	searchBy := KeySearchParam(searchObj)
	searchBy = *searchBy.WithAlias("alias01")
	...
	searchBy = KeySearchParam(searchObj)
	searchBy = *searchBy.WithEscape("Key16June")
	listKeysOptions = &ListKeysOptions{
		Search: searchBy,
	}
	...
	// Case 3: User using name exact value:
	listKeyResponse = []byte(`{
		"metadata":
		{
		  "collectionType": "application/vnd.ibm.kms.key+json",
	...
	searchBy = KeySearchParam(searchObj)
	searchBy = *searchBy.WithName()
	searchBy = *searchBy.WithExact("Key16JuneNew")
	listKeysOptions = &ListKeysOptions{
		Search: searchBy,
	}
