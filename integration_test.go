...
func TestWrapUnwrapWithKeyVersion(t *testing.T) {
	assert := assert.New(t)
	...
	wrapWithKeyVersion, err := c.WrapWithKeyVersion(ctx, crk.ID, nil, nil)
	assert.NoError(err)
	...
	assert.NoError(err)
	assert.EqualValues(getKeyVersion.KeyVersion[0].ID, wrapWithKeyVersion.KeyVersionDetails.ID)
