...
// GetPolicy retrieves a policy by Key idOrAlias. This function is
...
// SetPolicy updates a policy resource by specifying the idOrAlias of the key and
...
// GetPolicies retrieves all policies details associated with a Key idOrAlias.
...
func (c *Client) getPolicy(ctx context.Context, idOrAlias, policyType string, policyresponse *Policies) error {
	req, err := c.newRequest("GET", fmt.Sprintf("keys/%s/policies", idOrAlias), nil)
	if err != nil {
		return err
	}
...
// SetRotationPolicy updates the rotation policy associated with a key by specifying key idOrAlias and rotation interval.
// For more information can refer the Key Protect docs in the link below:
...
// SetDualAuthDeletePolicy updates the dual auth delete policy by passing the key idOrAlias and enable detail
// For more information can refer the Key Protect docs in the link below:
...
// SetPolicies updates all policies of the key or a single policy by passing key id or Alias.
